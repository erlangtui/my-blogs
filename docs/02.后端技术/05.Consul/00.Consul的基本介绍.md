...
　　理想情况下，每当新节点出现在您的数据中心时，它就会自动加入Consul集群，无需人工干预。 Consul通过启用AWS，Google Cloud或Azure中的实例的自动发现功能，使用给定的标签 key/value来促进自动加入。 要使用集成，请将retry_join_ec2，retry_join_gce或retry_join_azure嵌套对象添加到您的Consul配置文件。 这将允许新的节点加入集群，而不需要任何硬编码的配置。 或者，您可以在启动时使用-join选项或start_join设置以及其他已知Consul代理的硬编码地址加入集群。
