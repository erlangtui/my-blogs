...
* 尝试从消费者队列中取出一个 goroutine，如果能够能够取到，则说明当前管道是非缓冲管道（见下方提示）

::: tips
对于缓冲队列，当消费队列不为空时，说明管道中没有数据；当生产队列不为空时，说明管道已经满了；不可能同时存在消费队列和生产队列都不为空的情况？？？
对于非缓冲队列，当消费队列不为空时，说明此时生产队列为空，消费队列被阻塞等待中；当生产队列不为空时，说明消费队列为空，生产队列被阻塞等待中；
:::
::: details
```go
